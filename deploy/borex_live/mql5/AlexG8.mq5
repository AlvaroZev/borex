//+------------------------------------------------------------------+
//| AlexG8.mq5                                                       |
//| Ports core alexg8 path (= alexg7aligned, session=all):           |
//|   close-in D/W AOI, ghost fill at scaled SL, any session         |
//| NOT bit-identical to Python (AOI clustering / multi-pair /       |
//| risk-include-commission sizing differ). Single-chart only.       |
//+------------------------------------------------------------------+
#property copyright "borex"
...
input int    InpMagic         = 77008;
input bool   InpOnlyOverlap   = false;  // alexg8 default: all sessions
...
void ResolveGhost(const MqlRates &bar, const int bar_index)
  {
   ...
   bool ok = false;
   if(g_ghost.dir > 0)
      ok = trade.Buy(InpLots, _Symbol, 0.0, new_sl, new_tp, "alexg8");
   else
      ok = trade.Sell(InpLots, _Symbol, 0.0, new_sl, new_tp, "alexg8");
...
int OnInit()
  {
   trade.SetExpertMagicNumber(InpMagic);
   ClearGhost();
   Print("AlexG8 ready on ", _Symbol,
         " MinRR=", InpMinRR, " GhostMult=", InpGhostSlMult,
         " OnlyOverlap=", InpOnlyOverlap);
